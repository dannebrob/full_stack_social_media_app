...
const PinDetail = ({ user }) => {
  ...
  const fetchPinDetails = () => {
    ...
    if (query) {
      client.fetch(`${query}`).then((data) => {
        ...
        console.log(data);
        if (data[0]) {
          const firstData = pinDetailMorePinQuery(data[0]);
          client.fetch(firstData).then((res) => {
            setPins(res);
          });
        }
      });
  ...
  return (
    <>
        ...
        <div className="w-full p-5 flex-1 xl:min-w-620 ">
          ...
          >
            <img
              ...
              alt="user-profile"
            />
            <p className="font-semibold capitalize p-4">
          ...
          <div className="flex fle-wrap m-5 gap-3 ">
      ...
      {pins?.length > 0 ? (
        <>
          <h2 className="text-center text-bold text-2xl mt-8 mb-4">
            More like this{" "}
          </h2>
